#!/usr/bin/env bash
# =============================================================================
# 16-mistral-ai-loadbalancer.sh — Mistral AI Load Balancer Tools (Octavia)
# =============================================================================
# Installs Octavia load balancer management tools for the Mistral AI OpenStack
# agent. Provides complete LB lifecycle management with async state handling,
# health monitoring, and member management with transaction rollback support.
#
# What it installs:
#   - Octavia load balancer tools module (tools/loadbalancer.py)
#   - Load balancer lifecycle management (create, get, list, delete)
#   - Listener management with protocol support (HTTP, HTTPS, TCP)
#   - Pool creation with load balancing algorithms (ROUND_ROBIN, etc.)
#   - Health monitor configuration (HTTP, HTTPS, TCP, PING)
#   - Member management with weight and subnet support
#   - Async state management with wait_for_load_balancer
#   - Transaction rollback integration with cascade delete
#
# Tool Functions:
#   - list_load_balancers, get_load_balancer, create_load_balancer
#   - wait_for_load_balancer, create_lb_listener, create_lb_pool
#   - create_health_monitor, create_lb_member, delete_load_balancer
#   - list_lb_listeners, list_lb_pools, list_lb_members
#
# Prerequisites:
#   - 13-mistral-ai-core.sh (core agent installation)
#   - 11-octavia.sh (Octavia load balancing service)
#
# Usage:
#   sudo bash 16-mistral-ai-loadbalancer.sh           # Install LB tools
#   sudo bash 16-mistral-ai-loadbalancer.sh --uninstall # Remove LB tools
#
...
create_loadbalancer_tools() {
    echo "Creating tools/loadbalancer.py..."
    cat > "$INSTALL_DIR/tools/loadbalancer.py" << 'EOF'
import json
import time
...
EOF
    chown "$SERVICE_USER:$SERVICE_USER" "$INSTALL_DIR/tools/loadbalancer.py"
}

# ── Uninstall Mode ───────────────────────────────────────────────────────────

if [[ "${1:-}" == "--uninstall" ]]; then
    echo "Removing Mistral AI load balancer tools..."
    
    # Remove load balancer tools module
    rm -f "$INSTALL_DIR/tools/loadbalancer.py"
    
    # Remove any load balancer-related logs or cache
    find "$INSTALL_DIR" -name "*loadbalancer*" -type f -delete 2>/dev/null || true
    find "$INSTALL_DIR" -name "*octavia*" -type f -delete 2>/dev/null || true
    
    echo "Mistral AI load balancer tools removed"
    exit 0
fi

# ── Main Installation ────────────────────────────────────────────────────────

echo "Installing Mistral AI Load Balancer Tools (Octavia)..."
...
# Create load balancer tools
create_loadbalancer_tools

echo ""
echo "✓ Mistral AI Load Balancer Tools installed"
echo ""
echo "Features added:"
echo "  • Load balancer lifecycle (create, get, list, delete)"
echo "  • Listener management with protocol support"
echo "  • Pool creation with load balancing algorithms"
echo "  • Health monitor configuration"
echo "  • Member management with weight support"
echo "  • Async state management with wait_for_load_balancer"
echo "  • Transaction rollback integration (cascade delete)"
echo ""
echo "Next: Run 17-mistral-ai-quota.sh for quota and cost tools"